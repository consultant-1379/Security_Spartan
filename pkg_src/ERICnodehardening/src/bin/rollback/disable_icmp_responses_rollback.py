...
#
# ********************************************************************
# Name      : disable_icmp_responses_rollback.py
# Purpose   : This script rolls back icmp response.
# Author    : ZKATSHR
# Reason    : EQEV-131365
# ********************************************************************
"""
...
def rollback_icmpresponse():
    """"This function sets the icmp response value to its default state"""
    try:
        line = subprocess.check_output("cat /etc/sysctl.conf ", shell=True, stderr=subprocess.PIPE)
        if 'net.ipv4.icmp_ignore_bogus_error_responses=0' in line:
            print"\nCustomized value found\n"
            logging.info("Customized value found\n")
        elif 'net.ipv4.icmp_ignore_bogus_error_responses=1' not in line:
            print"\nGranular Node Hardening is not applied for icmp response\n"
            logging.info('Granular Node Hardening is not applied for icmp response\n')
        elif 'net.ipv4.icmp_ignore_bogus_error_responses=1' in line:
            os.system("sed -i '/net.ipv4.icmp_ignore_bogus_error_responses=1/d' /etc/sysctl.conf")
            print"\nSuccessfully icmp response has been rolled back\n"
            logging.info('Successfully icmp response has been rolled back\n')
    except (subprocess.CalledProcessError, OSError):
        logging.warning("Granular Node Hardening is not detected for icmp response")
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_icmp_responses  | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_icmp_responses | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_icmp_responses | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for icmp response\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for icmp response \n"
        logging.info('Granular Node Hardening not detected for icmp response\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_disable_icmp_responses_rollback.log'
...
% FNAME,format=FORMAT_STRING)
    ...
    SCRIPT_NAME = 'disable_icmp_responses_rollback.py'
