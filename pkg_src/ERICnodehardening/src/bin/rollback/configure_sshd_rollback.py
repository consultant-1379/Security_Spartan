...
#
# ********************************************************************
# Name      : configure_sshd_rollback.py
# Purpose   : This script sets AgentForwarding to its default state..
# Author    : ZKATSHR
# Reason    : EQEV-101907
# ********************************************************************
"""
...
def agent_fwdng():
    """This function rollback the AgentForwarding"""
    try:
        file_name = "/etc/ssh/sshd_config"
        data = subprocess.check_output("cat /etc/ssh/sshd_config \
| grep AllowAgentForwarding ", shell=True, stderr=subprocess.PIPE)
        if "AllowAgentForwarding no" in data :
            print"\nApplying rollback for AgentForwarding. . . . . .\n"
            logging.info('Applying rollback for AgentForwarding\n')
            with open(file_name) as file:
                newtext = file.read().replace('AllowAgentForwarding no', \
'#AllowAgentForwarding yes', 1)
            with open(file_name, 'w') as file:
                file.write(newtext)
                file.close()
            print "\n****Successfully rolled back Granular Node \
Hardening of disabling Agent Forwarding****\n"
            logging.info('Successfully rolled back Granular Node \
Hardening of disabling Agent Forwarding')
            print "\nRestarting the SSHD service. . . . . .\n"
            logging.info('Restarting the SSHD service\n')
            os.system("systemctl restart sshd")
        elif "#AllowAgentForwarding yes" in data:
...
for disabling Agent Forwarding\n"
...
log/Apply_NH_Logs/Manual_Exec/ | grep configure_sshd  | sort -n | tail -1 \
...
granular_nh_logs_time_stamp > rollback_logs_time_stamp:
        ...
        print"\n Granular Node Hardening not detected for AgentForwarding \n"
        logging.info('Granular Node Hardening not detected for AgentForwarding\n')
...
Apply_NH_Logs/Manual_Exec/ | grep configure_sshd | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not present for AgentForwarding\n"
                logging.info('Granular Node Hardening log is not present for AgentForwarding\n')
            else:
                print"\nGranular Node Hardening log is present for AgentForwarding\n"
                logging.info('Granular Node Hardening log is present for AgentForwarding\n')
                checking_latestlog()
        elif nh_check() == 1:
...
if __name__ == "__main__":
    ...
    FNAME = TIMESTR + '_configure_sshd_rollback.log'
    ...
    SCRIPT_NAME = 'configure_sshd_rollback.py'
