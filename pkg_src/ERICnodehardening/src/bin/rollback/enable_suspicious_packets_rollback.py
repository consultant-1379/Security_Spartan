...
#
# ********************************************************************
# Name      : enable_suspicious_packets_rollback.py
# Purpose   : This script rolls back suspicious packets.
# Author    : ZKATSHR
# Reason    : EQEV-131365
# ********************************************************************
"""
...
def rollback_suspicious_packets():
    """"This function sets the suspicious packets value to its default state"""
    try:
        line = subprocess.check_output("cat /etc/sysctl.conf ", shell=True, stderr=subprocess.PIPE)
        if 'net.ipv4.conf.all.log_martians=0' in line and 'net.ipv4.conf.default.log_martians=0' in line:
            print"\nCustomized value found\n"
            logging.info("Customized value found\n")
        elif 'net.ipv4.conf.all.log_martians=1' not in line and 'net.ipv4.conf.default.log_martians=1' not in line:
            print"\nGranular Node Hardening is not applied for suspicious packets\n"
            logging.info('Granular Node Hardening is not applied for suspicious packets\n')
        elif 'net.ipv4.conf.all.log_martians=1' in line and 'net.ipv4.conf.default.log_martians=1' in line:
            ...
            print"\nSuccessfully suspicious packets has been rolled back\n"
            logging.info('Successfully suspicious packets has been rolled back\n')
    except (subprocess.CalledProcessError, OSError):
        logging.warning("Granular Node Hardening is not detected for suspicious packets")
...
log/Apply_NH_Logs/Manual_Exec/ | grep enable_suspicious_packets  | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep enable_suspicious_packets  | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep enable_suspicious_packets | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for suspicious packets\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for suspicious packets \n"
        logging.info('Granular Node Hardening not detected for suspicious packets\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_enable_suspicious_packets_rollback.log'
...
% FNAME,format=FORMAT_STRING)
    ...
    SCRIPT_NAME = 'enable_suspicious_packets_rollback.py'
