...
#
# ********************************************************************
# Name      : ensure_kernel_module_rollback.py
# Purpose   : Rollback the kernel module loading and unloading that are collected.
# Author    : ZNITGUP
# Reason    : EQEV-115472
# ********************************************************************
"""
...
def rollback_kernel_module():
    '''Remove customized rules file from /etc/audit/rules.d/ dir'''
    try:
        os.remove('/etc/audit/rules.d/50-modules.rules')
        print("INFO: Removing  kernel module system configuration file")
        logging.info("INFO: Removing kernel module system configuration file")
    except OSError:
        print("Kernel module system configuration file is not found!!")
        logging.info("Kernel module system configuration file is not found!!")
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_kernel_module | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_kernel_module | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_kernel_module | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for kernel module\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for kernel module \n"
        logging.info('Granular Node Hardening not detected for kernel module\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_ensure_kernel_module_rollback.log'
    ...
    SCRIPT_NAME = 'ensure_kernel_module_rollback.py'
