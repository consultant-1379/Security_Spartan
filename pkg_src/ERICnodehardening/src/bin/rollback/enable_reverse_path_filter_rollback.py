...
#
# ********************************************************************
# Name      : enable_reverse_path_filter_rollback.py
# Purpose   : This script rolls back reverse path filter.
# Author    : ZKATSHR
# Reason    : EQEV-131365
# ********************************************************************
"""
...
def rollback_pathfilter():
    ...
    try:
        ...
        if 'net.ipv4.conf.all.rp_filter=0' in line and 'net.ipv4.conf.default.rp_filter=0' in line:
            print"\nCustomized value found\n"
            logging.info("Customized value found\n")
        elif 'net.ipv4.conf.all.rp_filter=1' not in line and 'net.ipv4.conf.default.rp_filter=1' not in line:
            print"\nGranular Node Hardening is not applied for reverse path filter\n"
            logging.info('Granular Node Hardening is not applied for reverse path filter\n')
        elif 'net.ipv4.conf.all.rp_filter=1' in line and 'net.ipv4.conf.default.rp_filter=1' in line:
            ...
            print"\nSuccessfully reverse path filter has been rolled back\n"
            logging.info('Successfully reverse path filter has been rolled back\\n')
    except (subprocess.CalledProcessError, OSError):
        logging.warning("Granular Node Hardening is not detected for reverse path filter")
...
log/Apply_NH_Logs/Manual_Exec/ | grep enable_reverse_path_filter  | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep enable_reverse_path_filter  | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep enable_reverse_path_filter | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for secure icmp\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for reverse path filter \n"
        logging.info('Granular Node Hardening not detected for reverse path filter\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_enable_reverse_path_filter_rollback.log'
...
% FNAME,format=FORMAT_STRING)
    ...
    SCRIPT_NAME = 'enable_reverse_path_filter_rollback.py'
