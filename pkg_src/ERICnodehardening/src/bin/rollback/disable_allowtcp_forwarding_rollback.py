...
#
# ********************************************************************
# Name      : disable_allowtcp_forwarding_rollback.py
# Purpose   : This script sets Tcp_Forwarding to its default state
# Author    : ZKATSHR
# Reason    : EQEV-101907
# ********************************************************************
"""
...
def allowtcpforwarding():
    """This function rollback the TcpForwarding"""
    try:
...
| grep -m1 AllowTcpForwarding", shell=True, stderr=subprocess.PIPE)
        if "AllowTcpForwarding no" in data:
            print"\nApplying rollback for TcpForwarding. . . . \n"
            logging.info('Applying rollback for TcpForwarding\n')
            with open(file_name, 'r') as file1:
                newtext = file1.read().replace('AllowTcpForwarding no', \
...
Hardening of disabling  AllowTcpForwarding****\n"
            logging.info('Successfully rolled back Granular Node \
Hardening of disabling  AllowTcpForwarding\n')
        elif "#AllowTcpForwarding yes" in data:
            print"\nGranular Node Hardening is not applied \
for disabling  AllowTcpForwarding\n"
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_AllowTcp_Forwarding  | sort -n | tail -1 \
...
granular_nh_logs_time_stamp > rollback_logs_time_stamp:
        ...
        print"\n Granular Node Hardening is not detected for TcpForwarding \n"
        logging.info('Granular Node Hardening is not detected for TcpForwarding\n')

def log_file_presence():
    """This function check whether log for particular feature present or not"""
    try:
        if nh_check() == 0:
            data=subprocess.check_output("find /ericsson/security/log/\
Apply_NH_Logs/Manual_Exec/ | grep disable_AllowTcp_Forwarding | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\n Granular Node Hardening log is not present for TcpForwarding \n"
                logging.info('Granular Node Hardening log is not present for TcpForwarding\n')
            else:
                print"\nGranular Node Hardening log is present for TcpForwarding\n"
                logging.info('Granular Node Hardening log is present for TcpForwarding\n')
                checking_latestlog()
        elif nh_check() == 1:
...
server, rollback cannot be applied***\n"
        ...
        logging.ingo('Granular Node Hardening not detected \n')

if __name__ == "__main__":

    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_disable_allowtcp_forwarding_rollback.log'
    ...
    SCRIPT_NAME = 'disable_allowtcp_forwarding_rollback.py'
