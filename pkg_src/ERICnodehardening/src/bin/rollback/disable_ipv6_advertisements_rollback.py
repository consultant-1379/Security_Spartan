...
#
# ********************************************************************
# Name      : disable_ipv6_advertisements_rollback.py
# Purpose   : This script rolls back ipv6 advertisements.
# Author    : ZKATSHR
# Reason    : EQEV-131365
# ********************************************************************
"""
...
def rollback_ipv6advertisements():
    """"This function sets the ipv6 advertisements value to its default state"""
    try:
        line = subprocess.check_output("cat /etc/sysctl.conf ", shell=True, stderr=subprocess.PIPE)
        if 'net.ipv6.conf.all.accept_ra=1' in line and 'net.ipv6.conf.default.accept_ra=1' in line:
            print"\nCustomized value found\n"
            logging.info("Customized value found\n")
        elif 'net.ipv6.conf.all.accept_ra=0' not in line and 'net.ipv6.conf.default.accept_ra=0' not in line:
            print"\nGranular Node Hardening is not applied for ipv6 advertisements\n"
            logging.info('Granular Node Hardening is not applied for ipv6 advertisements\n')
        elif 'net.ipv6.conf.all.accept_ra=0' in line and 'net.ipv6.conf.default.accept_ra=0' in line:
            ...
            print"\nSuccessfully ipv6 advertisements has been rolled back\n"
            logging.info('Successfully ipv6 advertisement has been rolled back\n')
    except (subprocess.CalledProcessError, OSError):
        logging.warning("Granular Node Hardening is not detected for ipv6 advertisements")
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_ipv6_advertisements  | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_ipv6_advertisements  | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_ipv6_advertisements | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for ipv6 advertisements\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for ipv6 advertisements \n"
        logging.info('Granular Node Hardening not detected for ipv6 advertisements\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_disable_ipv6_advertisements_rollback.log'
...
% FNAME,format=FORMAT_STRING)
    ...
    SCRIPT_NAME = 'disable_ipv6_advertisements_rollback.py'
