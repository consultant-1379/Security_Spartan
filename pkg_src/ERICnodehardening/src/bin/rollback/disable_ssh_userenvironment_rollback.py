...
#
# ********************************************************************
# Name      : disable_ssh_userenvironment_rollback.py
# Purpose   : This script reset PermitUserEnvironment to its default state
# Author    : ZKATSHR
# Reason    : EQEV-101907
# ********************************************************************
"""
...
def disable_ssh_userenvironment():
    """This function rollbacks the disable_ssh_userenvironment script """
    try:
        data1=subprocess.check_output("cat /etc/ssh/sshd_config \
| grep PermitUserEnvironment", shell=True, stderr=subprocess.PIPE).strip()
        if data1 == "#PermitUserEnvironment no":
            print"\nGranular Node Hardening is not applied for \
disabling of permit user environment\n"
            logging.info('Granular Node Hardening is not applied for \
disabling of permit user environment\n')
        elif data1 == "PermitUserEnvironment no":
            print"\nApplying rollback for disabling user environment. . . . .\n"
            logging.info('Applying rollback for disabling user environment\n')
            os.system("sed -i '/PermitUserEnvironment no/d' /etc/ssh/sshd_config")
            os.system("sed -i '110 i #PermitUserEnvironment no ' /etc/ssh/sshd_config")
            print "\nRestarting the SSHD service. . . . . .\n"
            logging.info('Restarting the SSHD service\n')
            os.system("systemctl restart sshd")
            print"\n****Successfully rolled back Granular Node \
Hardening of disabling of permit user environment****\n"
            logging.info('Successfully rolled back Granular Node \
Hardening of disabling of permit user environment\n')
        elif "PermitUserEnvironment yes" in data1:
...
Apply_NH_Logs/Manual_Exec/ | grep disable_ssh_userenvironment  | sort -n | tail -1 \
...
| cut -c 10-15", shell=True, stderr=subprocess.PIPE)
        ...
        print"\n Granular Node Hardening not detected for disabling user environment\n"
        logging.info('Granular Node Hardening not detected for disabling user environment\n')

def log_file_presence():
    """This function check whether granular log for particular feature present or not"""
    try:
        if nh_check() == 0:
            data = subprocess.check_output("find /ericsson/security/log/Apply_NH_Logs/\
Manual_Exec/ | grep disable_ssh_userenvironment | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for disable_ssh_userenvironment\n"
                logging.info('Granular Node Hardening log is not \
present for disabling user environment\n')
            else:
                print"\nGranular Node Hardening log is present for \
disabling user environment\n"
                logging.info('Granular Node Hardening log is present \
for disabling user environment\n')
                checking_latestlog()
        elif nh_check() == 1:
...
server, rollback cannot be applied***\n"
        ...
        print"\nGranular Node Hardening is not detected for disabling user environment\n"
        logging.info('Granular Node Hardening is not detected for disabling user environment\n')

if __name__ == "__main__":

    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_disable_ssh_userenvironment_rollback.log'
    ...
    SCRIPT_NAME = 'disable_ssh_userenvironment_rollback.py'
