...
#
# ********************************************************************
# Name      : ensure_system_network_rollback.py
# Purpose   : Rollback system network environment that are collected.
# Author    : ZNITGUP
# Reason    : EQEV-115472
# ********************************************************************
"""
...
def rollback_system_network():
    '''Remove customized rules file from /etc/audit/rules.d/ dir'''
    try:
        os.remove('/etc/audit/rules.d/50-system_local.rules')
        print("INFO: Removing system network configuration file")
        logging.info("INFO: Removing system network configuration file")
    except OSError:
        print("System network configuration file is not found!!")
        logging.info("System network configuration file is not found!!")
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_system_network  | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_system_network  | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_system_network | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for system network\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for system network \n"
        logging.info('Granular Node Hardening not detected for system network\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_ensure_system_network_rollback.py.log'
    ...
    SCRIPT_NAME = 'ensure_system_network_rollback.py.py'
