...
#
# ********************************************************************
# Name      : set_umask_rollback.py
# Purpose   : This script resets umask to its default value.
# Author    : ZKATSHR
# Reason    : EQEV-115470
# ********************************************************************
"""
...
def revert_umask_all():
    """This function is to set the umask value to its default state"""
    try:
        command = "cat /etc/profile"
        data = subprocess.check_output(command, shell=True, stderr=subprocess.PIPE).strip().split()
        if 'umask' in data and '002' in data:
            ...
            print"\nApplying rollback on umask feature\n"
            logging.info('Applying rollback on umask feature\n')
            with open('/etc/profile', 'r') as fin:
                newtext = fin.read()
            if 'umask 027' in newtext:
                newtext = newtext.replace('umask 027' , 'umask 002')
            with open('/etc/profile', 'w') as fout:
                fout.write(newtext)
                fout.close()
            print "\nSuccessfully reverted umask configuration for the OS users\n"
            logging.info('Successfully reverted configured umask for OS users')
    except (IOError, subprocess.CalledProcessError):
        print"\n Granular Node Hardening not detected for umask \n"
        logging.info('Granular Node Hardening not detected for umask\n')
...
log/Apply_NH_Logs/Manual_Exec/ | grep set_umask  | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep set_umask | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep set_umask | sort -n \
...
present for umask\n')
                ...
                print"\nGranular Node Hardening log are present for umask\n"
                logging.info('Granular Node Hardening log is present for umask\n')
                checking_latestlog()
        elif nh_check() == 1:
...
if __name__ == "__main__":
    ...
    FNAME = TIMESTR + '_set_umask_rollback.log'
...
% FNAME,format=FORMAT_STRING)
    ...
    SCRIPT_NAME = 'set_umask_rollback.py'
