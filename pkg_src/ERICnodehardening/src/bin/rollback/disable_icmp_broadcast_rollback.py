...
#
# ********************************************************************
# Name      : disable_icmp_broadcast_rollback.py
# Purpose   : This script rolls back icmp broadcast.
# Author    : ZKATSHR
# Reason    : EQEV-131365
# ********************************************************************
"""
...
def rollback_icmpbroadcast():
    """"This function sets the icmp broadcast value to its default state"""
    try:
        line = subprocess.check_output("cat /etc/sysctl.conf ", shell=True, stderr=subprocess.PIPE)
        if 'net.ipv4.icmp_echo_ignore_broadcasts=0' in line:
            print"\nCustomized value found\n"
            logging.info("Customized value found\n")
        elif 'net.ipv4.icmp_echo_ignore_broadcasts=1' not in line:
            print"\nGranular Node Hardening is not applied for icmp broadcast\n"
            logging.info('Granular Node Hardening is not applied for icmp broadcast\n')
        elif 'net.ipv4.icmp_echo_ignore_broadcasts=1' in line:
            os.system("sed -i '/net.ipv4.icmp_echo_ignore_broadcasts=1/d' /etc/sysctl.conf")
            print"\nSuccessfully icmp broadcast has been rolled back\n"
            logging.info('Successfully icmp broadcast has been rolled back\n')
    except (subprocess.CalledProcessError, OSError):
        logging.warning("Granular Node Hardening is not detected for icmp broadcast")
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_icmp_broadcast  | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_icmp_broadcast  | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_icmp_broadcast | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for icmp broadcast\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for icmp broadcast\n"
        logging.info('Granular Node Hardening not detected for icmp broadcast\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_disable_icmp_broadcast_rollback.log'
...
% FNAME,format=FORMAT_STRING)
    ...
    SCRIPT_NAME = 'disable_icmp_broadcast_rollback.py'
