...
#
# ********************************************************************
# Name      : set_password_aging_rollback.py
# Purpose   : This script resets the password aging to its default value.
# Author    : ZKATSHR
# Reason    : EQEV-115470
# ********************************************************************
"""
...
def check_password_aging():
    """This function set the password aging to its default state"""
    try:
        with open('/etc/login.defs') as fin:
            newtext = fin.read()
        if 'PASS_MAX_DAYS\t60\n' in newtext:
            newtext = newtext.replace('PASS_MAX_DAYS\t60\n' , 'PASS_MAX_DAYS\t99999\n')
        with open('/etc/login.defs', 'w') as fout:
            fout.write(newtext)
            fout.close()
        print"\nSuccessfully rolled back password aging feature\n"
        logging.info('Successfully rolled back password aging feature')
...
log/Apply_NH_Logs/Manual_Exec/ | grep set_password_aging  | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep set_password_aging | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep set_password_aging | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for password aging\n"
                logging.info('Granular Node Hardening log is not \
present for password aging\n')
            else:
                print"\nGranular Node Hardening log are present for password aging\n"
                logging.info('Granular Node Hardening log is present for password aging\n')
                checking_latestlog()
        elif nh_check() == 1:
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for password aging \n"
        logging.info('Granular Node Hardening not detected for password aging\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_set_password_aging_rollback.log'
 ...
 % FNAME,format=FORMAT_STRING)
    ...
    SCRIPT_NAME = 'set_password_aging_rollback.py'
