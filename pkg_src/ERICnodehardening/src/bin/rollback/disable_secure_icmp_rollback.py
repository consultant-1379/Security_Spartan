...
#
# ********************************************************************
# Name      : disable_secure_icmp_rollback.py
# Purpose   : This script rolls back secure icmp.
# Author    : ZKATSHR
# Reason    : EQEV-131365
# ********************************************************************
"""
...
def rollback_secureicmp():
    """"This function sets the secure icmp value to its default state"""
    try:
        line = subprocess.check_output("cat /etc/sysctl.conf ", shell=True, stderr=subprocess.PIPE)
        if 'net.ipv4.conf.all.secure_redirects=1' in line and 'net.ipv4.conf.default.secure_redirects=1' in line:
            print"\nCustomized value found\n"
            logging.info("Customized value found\n")
        elif 'net.ipv4.conf.all.secure_redirects=0' not in line and \
'net.ipv4.conf.default.secure_redirects=0' not in line:
            print"\nGranular Node Hardening is not applied for secure icmp\n"
            logging.info('Granular Node Hardening is not applied for secure icmp\n')
        elif 'net.ipv4.conf.all.secure_redirects=0' in line and 'net.ipv4.conf.default.secure_redirects=0' in line:
            ...
            print"\nSuccessfully secure icmp has been rolled back\n"
            logging.info('Successfully secure icmp has been rolled back\n')
    except (subprocess.CalledProcessError, OSError):
        logging.warning("Granular Node Hardening is not detected for secure icmp")
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_secure_icpm  | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_secure_icpm  | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_secure_icpm | sort -n \
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for secure icmp \n"
        logging.info('Granular Node Hardening not detected for secure icmp\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_disable_secure_icmp_rollback.log'
...
% FNAME,format=FORMAT_STRING)
    ...
    SCRIPT_NAME = 'disable_secure_icmp_rollback.py'
