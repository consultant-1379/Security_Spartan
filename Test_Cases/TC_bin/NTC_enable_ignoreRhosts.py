...
#
# ****************************************************************************
# Name      : NTC_enable_ignoreRhosts.py 
...
def ignoreRhosts():
    ...
    with open("/etc/ssh/sshd_config") as fin:
        if ("IgnoreRhosts yes" in data):
            newline = fin.read().replace('IgnoreRhosts yes', 'IgnoreRhosts no')
        else:
            os.system("echo IgnoreRhosts no >> /etc/ssh/sshd_config")

    if newline:
        with open("/etc/ssh/sshd_config", "w") as fout:
            fout.write(newline)

    status=enable_ignoreRhosts()
...
if __name__ == '__main__':
    ...
    fname = timestr + '_NTC_enable_ignoreRhosts.log'
