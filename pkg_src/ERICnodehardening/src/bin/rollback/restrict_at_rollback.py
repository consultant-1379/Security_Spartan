...
#
# ********************************************************************
# Name      : restrict_at_rollback.py
# Purpose   : This script resets the restrict at to its default value.
# Author    : ZKATSHR
# Reason    : EQEV-115470
# ********************************************************************
"""
...
def revert_restrict_at():
    """This function is to set the restrict at to its default state"""
    try:
        file_name = '/etc/at.allow'
        if not os.path.exists(file_name):
            ...
            print"\nApplying rollback on restrict at feature\n"
            logging.info('Applying rollback on restrict at feature\n')
            os.system("rm -rf /etc/at.allow")
            os.system("rm -rf /etc/at.deny")
            print "\nSuccessfully rolled back the restrict at feature\n"
            logging.info('Successfully rolled back the restrict at feature\n')
    except (IOError, subprocess.CalledProcessError):
        print"\n Granular Node Hardening not detected for restrict at feature \n"
        logging.info('Granular Node Hardening not detected for restrict at feature\n')
...
log/Apply_NH_Logs/Manual_Exec/ | grep restrict_at  | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep restrict_at | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep restrict_at | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for restrict at feature\n"
                logging.info('Granular Node Hardening log is not \
present for restrict at feature\n')
            else:
                print"\nGranular Node Hardening log are present for restrict at feature\n"
                logging.info('Granular Node Hardening log is present for restrict at feature\n')
                checking_latestlog()
        elif nh_check() == 1:
...
if __name__ == "__main__":
    ...
    FNAME = TIMESTR + '_restrict_at_rollback.log'
...
% FNAME,format=FORMAT_STRING)
    ...
    SCRIPT_NAME = 'restrict_at_rollback.py'
