...
#
# ********************************************************************
# Name      : disable_access_suid_rollback.py
# Purpose   : This script rolls back the permission manually.
# Author    : ZKATSHR
# Reason    : EQEV-115473
# ********************************************************************
"""
...
def rollback_root_uid():
    """This function enable the access to files with root suid."""
    try:
        if os.path.exists("/ericsson/security/log/Apply_NH_Logs/suid_list"):
            data = open("/ericsson/security/log/Apply_NH_Logs/suid_list", 'r').read().split("\n")
            for data1 in data:
                cmd1 = "chmod u+s %s 2>/dev/null"%(data1)
                status = subprocess.call(cmd1, shell=True)
                if status:
                    logging.warning('Unset SUID permission for the file %s , \
was unsuccessful.', data1)
                    check = True
                else:
                    logging.info('Unset SUID for the file %s', data1)
                    print 'Unset SUID for the file {} '.format(data1)
            if check:
                print "Unset SUID permission was unsuccessful for certain files , refer to the \
log file for further information on the files."
    except (IOError, subprocess.CalledProcessError):
        print"\n Granular Node Hardening is not detected for root suid\n"
        logging.info('Granular Node Hardening is not detected for root suid\n')
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_access_suid  | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_access_suid  | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_access_suid | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for suid access\n"
                logging.info('Granular Node Hardening log is not \
present for suid access\n')
            else:
                print"\nGranular Node Hardening log is present for suid access\n"
                logging.info('Granular Node Hardening log is present for suid access\n')
                checking_latestlog()
        elif nh_check() == 1:
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for suid access \n"
        logging.info('Granular Node Hardening not detected for suid access\n')

if __name__ == "__main__":

    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_disable_access_suid_rollback.log'
...
% FNAME,format=FORMAT_STRING)
    ...
    SCRIPT_NAME = 'disable_access_suid_rollback.py'
