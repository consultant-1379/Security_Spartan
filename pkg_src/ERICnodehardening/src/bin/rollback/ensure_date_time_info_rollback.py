...
#
# ********************************************************************
# Name      : ensure_date_time_info_rollback.py
# Purpose   : Rollback the modify date and time information that are collected.
# Author    : ZNITGUP
# Reason    : EQEV-115472
# ********************************************************************
"""
...
def rollback_date_time_info():
    '''Remove customized rules file from /etc/audit/rules.d/ dir'''
    try:
        os.remove('/etc/audit/rules.d/50-time_change.rules')
        print("INFO: Removing date time information  system configuration file")
        logging.info("INFO: Removing date time information  system  system configuration file")
    except OSError:
        print("Date time information  system configuration file is not found!!")
        logging.info("Date time information  system configuration file is not found!!")
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_date_time_info  | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_date_time_info  | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_date_time_info | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for date time information\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for date time information \n"
        logging.info('Granular Node Hardening not detected for date time information\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_ensure_date_time_info_rollback.py.log'
    ...
    SCRIPT_NAME = 'ensure_date_time_info_rollback.py.py'
