...
#
# ********************************************************************
# Name      : configure_icmp_rollback.py
# Purpose   : This script rolls back the configure icmp.
# Author    : ZKATSHR
# Reason    : EQEV-131365
# ********************************************************************
"""
...
def unblock_icmp_type(str):
    """This function blocks the icmp types listed"""
    block_status = commands.getoutput("firewall-cmd --remove-icmp-block=%s" % str)
    if block_status == 'success':
        return 'yes'
    else:
        return 'no'

def rollback_configure_icmp():
    """"This function sets the configure_icmp value to its default state"""
    try:
        ...
        for i in icmp_types:
            ...
            if icmp_status == 'no':
                print "\n**********The icmp-type: %s is already rolledback**********" % i
                logging.info("The icmp-type : %s is already rolled back", i)
            else:
                icmp_block_status = unblock_icmp_type(i)
                if icmp_block_status == 'yes':
                    print "\n*********Rolled back the icmp-type : %s" % i, "**********\n"
                    logging.info("Rolled back the icmp-type : %s", i)
                else:
                    print "Failed to rolled back the icmp-type : %s", i
                    logging.error("Failed to rolled back the icmp-type : %s", i)
        line = subprocess.check_output("cat /etc/sysctl.conf ", shell=True, stderr=subprocess.PIPE)
        if 'net.ipv4.icmp_echo_ignore_all = 0' not in line:
            print"\n*********Granular Node Hardening is not applied for configure icmp*********\n"
            logging.info('*********Granular Node Hardening is not applied for configure icmp*********\n')
        elif 'net.ipv4.icmp_echo_ignore_all = 0' in line:
            os.system("sed -i '/net.ipv4.icmp_echo_ignore_all = 0/d' /etc/sysctl.conf")
            print"\n*********Successfully rolled back configure icmp*********\n"
            logging.info('*********Successfully rolled back configure icmp*********\n')
    except (subprocess.CalledProcessError, OSError):
        logging.warning("Granular Node Hardening is not detected for configure icmp")
...
log/Apply_NH_Logs/Manual_Exec/ | grep configure_icmp  | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep configure_icmp  | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep configure_icmp | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for configure icmp\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for configure icmp\n"
        logging.info('Granular Node Hardening not detected for configure icmp\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_configure_icmp_rollback.log'
...
% FNAME,format=FORMAT_STRING)
    ...
    SCRIPT_NAME = 'configure_icmp_rollback.py'
