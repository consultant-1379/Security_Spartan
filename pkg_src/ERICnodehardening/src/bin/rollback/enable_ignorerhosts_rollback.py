...
#
# ********************************************************************
# Name      : enable_ignorerhosts_rollback.py
# Purpose   : This script resets the IgnoreRhosts to defalut state
# Author    : ZKATSHR
# Reason    : EQEV-101907
# ********************************************************************
"""
...
def ignorerhosts():
    """This function rollback enable_ignoreRhosts"""
    try:
        file_name = "/etc/ssh/sshd_config"
        data=subprocess.check_output("cat /etc/ssh/sshd_config \
| grep IgnoreRhosts", shell=True, stderr=subprocess.PIPE).strip()
        if data == "IgnoreRhosts yes":
            print"\nApplying rollback for ignore Rhosts. . . .\n"
            logging.info('Applying rollback for ignore Rhosts\n')
...
Hardening of IgnoreRhosts****\n"
            logging.info('Successfully rolled back Granular Node \
Hardening of IgnoreRhosts')
        elif data == "#IgnoreRhosts yes":
...
/log/Apply_NH_Logs/Manual_Exec/ | grep enable_ignoreRhosts  | sort -n | tail -1 \
...
and  granular_nh_logs_time_stamp > rollback_logs_time_stamp:
            ...
            print"\nGranular Node ardening is not applied for the feature\n"
            logging.info('Granular Node Hardening is not applied for the feature\n')
        else:
            print"\nGranular Node Hardening is not applied\n"
            logging.info('Granular Node Hardening is not applied\n')
    except (subprocess.CalledProcessError, IOError):
        print"\nGranular Node Hardening is not detected for ignore Rhosts\n"
        logging.info('Granular Node Hardening is not detected for ignore Rhosts\n')
...
| grep enable_ignoreRhosts | sort -n | tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\n Granular Node Hardening log is not present for enable_ignoreRhosts \n"
                logging.info('Granular Node Hardening log is not present for enable_ignoreRhosts')
            else:
                print"\nGranular Node Hardening log is present for enable_ignoreRhosts\n"
                logging.info('Granular Node Hardening log is present for enable_ignoreRhosts\n')
                checking_latestlog()
        elif nh_check() == 1:
...
if __name__ == "__main__":
    ...
    FNAME = TIMESTR + '_enable_ignorerhosts_rollback.log'
    ...
    SCRIPT_NAME = 'enable_ignorerhosts_rollback.py'
