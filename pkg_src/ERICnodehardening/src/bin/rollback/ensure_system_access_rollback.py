...
#
# ********************************************************************
# Name      : ensure_system_access_rollback.py
# Purpose   : Rollback the system mandatory access controls that are collected.
# Author    : ZNITGUP
# Reason    : EQEV-115472
# ********************************************************************
"""
...
def rollback_system_access():
    '''Remove customized rules file from /etc/audit/rules.d/ dir'''
    try:
        os.remove('/etc/audit/rules.d/50-MAC_policy.rules')
        print("INFO: Removing system access configuration file")
        logging.info("INFO: Removing system access configuration file")
    except OSError:
        print("System access configuration file is not found!!")
        logging.info("System access configuration file is not found!!")
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_system_access   | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_system_access | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_system_access | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for system access\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for system access \n"
        logging.info('Granular Node Hardening not detected for system access\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_ensure_system_access_rollback.log'
    ...
    SCRIPT_NAME = 'ensure_system_access_rollback.py'
