...
#
# ********************************************************************
# Name      : set_motd_banner_rollback.py
# Purpose   : This script resets the motd_banner to its default state
# Author    : ZKATSHR
# Reason    : EQEV-101907
# ********************************************************************
"""
...
def motd_banner():
    """This function rollback the motd_banner  """
    try:
        file_name = "/etc/motd"
        ...
        if os.stat(file_name).st_size != 0:
            print"\nApplying rollback for motd banner. . . . . .\n"
            logging.info('Applying rollback for motd banner\n')
            os.system("sed -ni '' /etc/motd")
            print "\nRestarting the SSHD service. . . . . .\n"
            logging.info('Restarting the SSHD service')
            os.system("systemctl restart sshd")
            print"\n***Successfully rolled back Granular Node \
Hardening for MOTD Banner****\n"
...
for MOTD Banner\n')
        ...
        print"\nError in Granular Node Hardening \
rollback for MOTD Banner\n"
        logging.info('Error in Granular Node Hardening \
rollback for MOTD Banner\n')
...
Apply_NH_Logs/Manual_Exec/ | grep set_motd_banner  | sort -n | tail -1 | cut -d '/' -f 7 \
...
granular_nh_logs_time_stamp > rollback_logs_time_stamp:
        ...
        print"\nGranular Node Hardening is not detected for motd banner\n"
        logging.info('Granular Node Hardening is not detected for motd banner\n')
...
| grep set_ssh_banner | sort -n | tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\n Granular Node Hardening log is not present for motd banner\n"
                logging.info('Granular Node Hardening log is not present for motd banner\n')
            else:
                print"\nGranular Node Hardening log is present for motd banner\n"
                logging.info('Granular Node Hardening log is present for motd banner\n')
                checking_latestlog()
        elif nh_check() == 1:
...
if __name__ == "__main__":
    ...
    FNAME = TIMESTR + '_set_motd_banner_rollback.log'
    ...
    SCRIPT_NAME = 'set_motd_banner_rollback.py'
