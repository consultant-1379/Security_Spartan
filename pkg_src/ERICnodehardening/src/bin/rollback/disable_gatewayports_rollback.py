...
#
# ********************************************************************
# Name      : disable_gatewayports_rollback.py
# Purpose   : This script rollbacks gateway ports to its default value
# Author    : ZKATSHR
# Reason    : EQEV-101907
# ********************************************************************
"""
...
def gateway_ports():
    """This function rollback disable_GatewayPorts script"""
    try:
        data = subprocess.check_output("cat /etc/ssh/ssh_config \
| grep GatewayPorts", shell=True, stderr=subprocess.PIPE)
        if "GatewayPorts no" in data:
            print"\nApplying rollback for disabling Gateway Ports. . . .\n"
            logging.info('Applying rollback for disabling Gateway Ports\n')
            os.system("sed -i '/GatewayPorts no/d' /etc/ssh/ssh_config")
            print "\nRestarting the SSHD service. . . . . .\n"
            logging.info('Restarting the SSHD service')
            os.system("systemctl restart sshd")
            print"\n****Successfully rolled back Granular Node \
Hardening of disabling  gateway ports****\n"
            logging.info('Successfully rolled back Granular Node \
Hardening of disabling  gateway ports\n')
    except subprocess.CalledProcessError:
        print"\nGranular Node Hardening is not applied for \
disabling gateway ports\n"
...
/log/Apply_NH_Logs/Manual_Exec/ | grep disable_GatewayPorts  | sort -n | tail -1 \
...
granular_nh_logs_time_stamp > rollback_logs_time_stamp:
        ...
        print"\n Granular Node Hardening is not detected for disabling Gateway ports\n"
        logging.info('Granular Node Hardening is not detected for disabling Gateway ports\n')
...
| grep disable_GatewayPorts | sort -n | tail -1",shell=True, \
stderr=subprocess.PIPE).split()
            if not data:
                print"\n Granular Node Hardening log is not present for \
disabling Gateway ports\n"
                logging.info('Granular Node Hardening log is not present \
for disabling Gateway ports\n')
            else:
                print"\nGranular Node Hardening log is present for disabling Gateway ports\n"
                logging.info('Granular Node Hardening log is present for disabling Gateway ports\n')
                checking_latestlog()
        elif nh_check() == 1:
...
if __name__ == "__main__":
    ...
    FNAME = TIMESTR + '_disable_gatewayports_rollback.log'
    ...
    SCRIPT_NAME = 'disable_gatewayports_rollback.py'
