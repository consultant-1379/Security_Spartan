...
#
# ********************************************************************
# Name      : ensure_file_deletion_rollback.py
# Purpose   : Rollback the file deletion events by users that are collected.
# Author    : ZNITGUP
# Reason    : EQEV-115472
# ********************************************************************
"""
...
def rollback_file_deletion():
    '''Remove customized rules file from /etc/audit/rules.d/ dir'''
    try:
        os.remove('/etc/audit/rules.d/50-deletion.rules')
        print("INFO: Removing file deletion  system configuration file")
        logging.info("INFO: Removing file deletion system configuration file")
    except OSError:
        print("File deletion system configuration file is not found!!")
        logging.info("File deletion system configuration file is not found!!")
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_file_deletion | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_file_deletion | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_file_deletion | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for file deletion\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for file deletion \n"
        logging.info('Granular Node Hardening not detected for file deletion\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_ensure_file_deletion_rollback.log'
    ...
    SCRIPT_NAME = 'ensure_file_deletion_rollback.py'
