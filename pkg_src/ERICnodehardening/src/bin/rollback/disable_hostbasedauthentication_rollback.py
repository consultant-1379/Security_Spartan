...
#
# ********************************************************************
# Name      : disable_hostbasedauthentication_rollback.py
# Purpose   : This script sets HostbasedAuthentication to its default state.
# Author    : ZKATSHR
# Reason    : EQEV-101907
# ********************************************************************
"""
...
def disable_hostbasedauthentication():
    """This function rollback the HostbasedAuthentication script"""
    try:
...
| grep -m1 HostbasedAuthentication", shell=True, stderr=subprocess.PIPE).strip()
        if data == "HostbasedAuthentication no":
            print"\nApplying rollback for HostbaseAuthentication. . . .\n"
            logging.info('Applying rollback for HostbaseAuthentication\n')
            with open(file_name, 'r') as fin:
                newconfig = fin.read().replace('HostbasedAuthentication no', \
...
Hardening of  disabling  HostBasedAuthentication****\n"
            logging.info('Successfully rolled back Granular Node \
Hardening of  disabling  HostBasedAuthentication\n')
        elif data == "#HostbasedAuthentication no":
...
disabling  HostbasedAuthentication\n"
...
log/Apply_NH_Logs/Manual_Exec/ | grep disable_hostbasedAuthentication  | sort -n \
| tail -1 | cut -d '/' -f 7 | cut -c 1-8", shell=True, stderr=subprocess.PIPE)
        granular_nh_logs_time_stamp = subprocess.check_output("find /ericsson/security/\
log/Apply_NH_Logs/Manual_Exec/ | grep disable_hostbasedAuthentication  | sort -n \
| tail -1 | cut -d '/' -f 7 | cut -c 10-15", shell=True, stderr=subprocess.PIPE)
...
| cut -c 10-15", shell=True, stderr=subprocess.PIPE)
        if granular_nh_logs_date_stamp > rollback_logs_date_stamp:
            disable_hostbasedauthentication()
...
Manual_Exec/ | grep disable_hostbasedAuthentication | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\n Granular Node Hardening log is not \
present for HostbasedAuthentication\n"
...
if __name__ == "__main__":
    ...
    FNAME = TIMESTR + '_disable_hostbaseauthentication_rollback.log'
    ...
    SCRIPT_NAME = 'disable_hostbasedauthentication_rollback.py'
