...
#
# ********************************************************************
# Name      : ensure_session_info_rollback.py
# Purpose   : Rollback audit session initiation information
#             are collected.
# Author    : ZNITGUP
# Reason    : EQEV-115472
# ********************************************************************
...
def rollback_session_info():
    '''Remove customized rules file from /etc/audit/rules.d/ dir'''
    try:
        os.remove('/etc/audit/rules.d/50-session.rules')
        print("INFO: Removing session inforamtion system configuration file ")
        logging.info("INFO: Removing session inforamtion system configuration file")
    except OSError:
        print("Session inforamtion system configuration file is not found!!")
        logging.info("Session inforamtion system configuration file is not found!!")
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_audit_session_info | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_audit_session_info | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_audit_session_info | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for session info\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for session info \n"
        logging.info('Granular Node Hardening not detected for session info\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_ensure_audit_session_info_rollback.log'
    ...
    SCRIPT_NAME = 'ensure_audit_session_info_rollback.py'
