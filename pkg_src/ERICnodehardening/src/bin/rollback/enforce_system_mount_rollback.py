...
#
# ********************************************************************
# Name      : enforce_system_mount_rollback.py
# Purpose   : Rollback the successful system mounts that are collected.
# Author    : ZNITGUP
# Reason    : EQEV-115472
# ********************************************************************
"""
...
def rollback_system_mount():
    '''Remove customized rules file from /etc/audit/rules.d/ dir'''
    try:
        os.remove('/etc/audit/rules.d/50-mounts.rules')
        print("INFO: Removing system mount configuration file")
        logging.info("INFO: Removing system mount configuration file")
    except OSError:
        print("System mount configuration file is not found!!")
        logging.info("System mount configuration file is not found!!")
...
log/Apply_NH_Logs/Manual_Exec/ | grep enforce_system_mount | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep enforce_system_mount | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep enforce_system_mount| sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for enforce system mount\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for enforce system mount \n"
        logging.info('Granular Node Hardening not detected for enforce system mount\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_enforce_system_mount_rollback.log'
    ...
    SCRIPT_NAME = 'enforce_system_mount_rollback.py'
