...
#
# *********************************************************************
# Name      : ensure_user_priviliged_cmd_rollback.py
# Purpose   : This script will rollback the use of privileged commands
# Author    : ZNITGUP
# Reason    : EQEV-115472
# *********************************************************************
"""
...
def rollback_user_priviliged_cmd():
    '''Remove customized rules file from /etc/audit/rules.d/ dir'''
    try:
        os.remove('/etc/audit/rules.d/50-privileged.rules')
        print("INFO: Removing user priviliged command system configuration file")
        logging.info("INFO: Removing user priviliged command system configuration file")
    except OSError:
        print("User priviliged command  system configuration file is not found!!")
        logging.info("User priviliged command system configuration file is not found!!")
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_user_priviliged_cmd | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_user_priviliged_cmd | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_user_priviliged_cmd | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for user priviliged cmd\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for user priviliged cmd \n"
        logging.info('Granular Node Hardening not detected for user priviliged cmd\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_ensure_user_priviliged_cmd_rollback.log'
    ...
    SCRIPT_NAME = 'ensure_user_priviliged_cmd_rollback.py'
