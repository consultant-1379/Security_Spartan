...
#
# *********************************************************************
# Name      : discretionary_access_control_rollback.py
# Purpose   : Rollback discretionary access control permission
#             modification events that are collected.
# Author    : ZNITGUP
# Reason    : EQEV-115472
# *********************************************************************
"""
...
def rollback_discretionary_access_control():
    '''Remove customized rules file from /etc/audit/rules.d/ dir'''
    try:
        os.remove("/etc/audit/rules.d/50-perm_mod.rules")
        print("INFO: Removing discretionary access control system configuration file")
        logging.info("INFO: Removing discretionary access control system configuration file")
    except OSError:
        print("Discretionary access control system configuration file is not found!!")
        logging.info("Discretionary access control system configuration file is not found!!")
...
log/Apply_NH_Logs/Manual_Exec/ | grep discretionary_access_control | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep discretionary_access_control | sort -n | tail -1 | cut -d '/' -f 7 \
...
| cut -c 10-15", shell=True, stderr=subprocess.PIPE)
        if granular_nh_logs_date_stamp > rollback_logs_date_stamp:
            rollback_discretionary_access_control()
...
log/Apply_NH_Logs/Manual_Exec/ | grep discretionary_access_control| sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for discretionary access control\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for discretionary access control \n"
        logging.info('Granular Node Hardening not detected for discretionary access control\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_discretionary_access_control_rollback.log'
    ...
    SCRIPT_NAME = 'discretionary_access_control_rollback.py'
