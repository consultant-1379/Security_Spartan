...
#
# ********************************************************************
# Name      : ensure_file_auth_rollback.py
# Purpose   : Rollback unsuccessful unauthorized file access
#             attempts that are collected.
# Author    : ZNITGUP
# Reason    : EQEV-115472
# ********************************************************************
"""
...
def rollback_file_auth():
    '''Remove customized rules file from /etc/audit/rules.d/ dir'''
    try:
        os.remove('/etc/audit/rules.d/50-access.rules')
        print("INFO: Removing file auth  system configuration file")
        logging.info("INFO: Removing file auth  system configuration file")
    except OSError:
        print("File auth  system configuration file is not found!!")
        logging.info("File auth  system configuration file is not found!!")
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_file_auth | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_file_auth | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_file_auth| sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for ensure file auth\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for file auth \n"
        logging.info('Granular Node Hardening not detected for file auth\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_ensure_file_auth_rollback.log'
    ...
    SCRIPT_NAME = 'ensure_file_auth_rollback.py'
