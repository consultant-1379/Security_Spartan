...
#
# ********************************************************************
# Name      : set_maxauthtries_rollback.py
# Purpose   : This script sets maxauthtries to default value
# Author    : ZKATSHR
# Reason    : EQEV-101907
# ********************************************************************
"""
...
def replace_maxauthtries():
    """This function rollback the set_maxauthtries script"""
    try:
...
| grep MaxAuthTries ", shell=True, stderr=subprocess.PIPE)
        if "MaxAuthTries 4" in data :
            print"\nApplying rollback for maxauthtries. . . . .\n"
            logging.info('Applying rollback for maxauthtries\n')
            with open(file_name) as fin:
                newtext = fin.read().replace('MaxAuthTries 4', '#MaxAuthTries 6')
...
for Maximum Authentication Tries parameter for SSH communication******\n"
            logging.info('Successfully rolled back Granular Node Hardening \
for Maximum Authentication Tries parameter for SSH communication\n')
        elif "#MaxAuthTries 6" in data:
            print"\nGranular Node Hardening is not applied for the \
MaxAuthTries parameter\n"
            logging.info('Granular Node Hardening is not applied for the \
MaxAuthTries parameter\n')
        elif "MaxAuthTries 4" not in data and "#MaxAuthTries 6" not in data :
            print"\n Customize value found we can not rollback \n"
            logging.info('Customize value found we can not rollback \n')
        else:
            print"\n Customize value found we cannot rollback \n"
            logging.info('Customize value found we cannot rollback \n')
...
/log/Apply_NH_Logs/Manual_Exec/ | grep set_maxauthtries  | sort -n | tail -1 \
...
granular_nh_logs_time_stamp > rollback_logs_time_stamp:
        ...
        print"\nGranular Node Hardening is not detected for maxauthtries\n"
        logging.info('Granular Node Hardening is not detected for maxauthtries\n')
...
| grep set_maxauthtries | sort -n | tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\n Granular Node Hardening log is not present for maxauthtries\n"
                logging.info('Granular Node Hardening log is not present for maxauthtries \n')
            else:
                print"\nGranular Node Hardening log is present for maxauthtries\n"
                logging.info('Granular NH log is present for maxauthtries\n')
                checking_latestlog()
        elif nh_check() == 1:
...
if __name__ == "__main__":
    ...
    FNAME = TIMESTR + '_set_maxauthtries_rollback.log'
    ...
    SCRIPT_NAME = 'set_maxauthtries_rollback.py'
