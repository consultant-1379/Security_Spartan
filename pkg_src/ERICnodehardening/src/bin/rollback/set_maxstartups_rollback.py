...
#
# ********************************************************************
# Name      : set_maxstartups_rollback.py
...
def replace_maxtstartup():
    """This function rollback the set_maxtstartup script"""
    try:
...
| grep MaxStartups ", shell=True, stderr=subprocess.PIPE)
        if "MaxStartups 10:30:60" in data :
            print"\nApplying rollback for set_maxstartups. . . . . . \n"
            logging.info('Applying rollback for set_maxstartups\n')
            with open(file_name) as fin:
                newtext = fin.read().replace('MaxStartups 10:30:60', '#MaxStartups 10:30:100 ')
...
for the MaxStartups for SSH communication*****\n"
            logging.info('Successfully rolled back Granular Node Hardening \
for the MaxStartups for SSH communication\n')
        elif "#MaxStartups 10:30:100" in data:
            print"\nGranular Node Hardening is not applied \
for the MaxStartups\n"
            logging.info('Granular Node Hardening is not applied \
for the MaxStartups\n')
        else:
            print"\nCustomize value found, can not rollback the feature\n"
            logging.info('Customize value found, can not rollback feature\n')
...
/log/Apply_NH_Logs/Manual_Exec/ | grep set_maxstartups  | sort -n | tail -1 \
...
granular_nh_logs_time_stamp > rollback_logs_time_stamp:
        ...
        print"\nGranular Node Hardening is not detected for maxstartups\n"
        logging.info('Granular Node Hardening is not detected for maxstartups\n')
...
| grep set_maxstartups | sort -n | tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\n Granular Node Hardening log is not present for maxstartups \n"
                logging.info('Granular Node Hardening log is not present for maxstartups \n')
            else:
                print"\nGranular Node Hardening log is present for maxstartups\n"
                logging.info('Granular Node Hardening log is present for maxstartups\n')
                checking_latestlog()
        elif nh_check() == 1:
...
if __name__ == "__main__":
    ...
    FNAME = TIMESTR + '_set_maxstartups_rollback.log'
    ...
    SCRIPT_NAME = 'set_maxstartups_rollback.py'
