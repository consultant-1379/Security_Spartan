...
#
# *********************************************************************
# Name      : ensure_user_group_info_rollback.py
# Purpose   : Rollback events that modify user/group information that are collected.
# Author    : ZNIGUP
# Reason    : EQEV-115472
# *********************************************************************
"""
...
def rollback_user_group_info():
    '''Remove customized rules files from /etc/audit/rules.d/ dir'''
    try:
        os.remove('/etc/audit/rules.d/50-identity.rules')
        print("INFO: Removing user group information system configuration file")
        logging.info("INFO: Removing user group information system configuration file")
    except OSError:
        print("User group information configuration file system is not found!!")
        logging.info("User group information system configuration file is not found!!")
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_user_group_info | sort -n | tail -1 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_user_group_info | sort -n | tail -1 | cut -d '/' -f 7 \
...
log/Apply_NH_Logs/Manual_Exec/ | grep ensure_user_group_info | sort -n \
| tail -1",shell=True, stderr=subprocess.PIPE).split()
            if not data:
                print"\nGranular Node Hardening log is not \
present for user group info\n"
...
server, rollback can not be applied******\n"
        ...
        print"\n Granular Node Hardening not detected for user group info \n"
        logging.info('Granular Node Hardening not detected for user group info\n')

if __name__ == "__main__":
    user_verification()
    TIMESTR = time.strftime("%Y%m%d-%H%M%S")
    FNAME = TIMESTR + '_ensure_user_group_info_rollback.log'
    ...
    SCRIPT_NAME = 'ensure_user_group_info_rollback.py'
